...
namespace Labixa.Controllers
{
    public class AccountController : BaseHomeController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    if (!user.Deleted)
                    {
                        ...
                        {
                            model.alert = "Tài khoản của bạn tạm thời bị vô hiệu hóa.";
                        }
                    }
                    else
                    {
                        model.alert = "Tài khoản của bạn bị vô hiệu hóa.";
                    }
                }
                else
                {
                    ModelState.AddModelError("", "Invalid username or password.");
                    model.alert = "Sai tên đăng nhập hoặc mật khẩu.";
                }
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, bool isAdmin=false)
        {
            ...
            if (ModelState.IsValid)
            {
                var role = SystemRoles.Guest;
                if (isAdmin == true)
                {
                    role = SystemRoles.Admin;
                }
                var user = new User() { UserName = model.UserName, RoleId = SystemRoles.Guest,EmailConfirmed=true, Activated = true };
                var result = await _userManager.CreateAsync(user, model.Password);
                user = await _userManager.FindByNameAsync(user.UserName);
                if (isAdmin && result.Succeeded)
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("MyProperty", "Profile");
                }
                else
                {
                    AddErrors(result);
            ...
            //if (ModelState.IsValid)
            //{
            //    var user = new User() { UserName = model.UserName, RoleId = SystemRoles.Guest, LastName = "11", Activated = true,DateOfBirth=DateTime.Now,Email="aa",EmailConfirmed=true,Address="â",Deleted=false,FirstName="b",Gender=Gender.Male };
            //    var result = await _userManager.CreateAsync(user, model.Password);
            //    if (result.Succeeded)
            //    {
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            else
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        return RedirectToAction("Index", "Profile", new { Message = ManageMessageId.SetPasswordSuccess });
                    }
                    else
                    {
                        AddErrors(result);
            ...
            // If we got this far, something failed, redisplay form
            return RedirectToAction("Index", "Profile", new { Message = "Có Lỗi Xảy Ra" });
            }
